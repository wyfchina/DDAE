...
| `NetworkStructure.Optimization` | 候选动作组合选择，Gurobi / OR-Tools adapter | 作为后续能力保留在底层架构中，不在网络结构评分工作台暴露。该能力未来应后移到 DDS&OP 多方案比较阶段：先用求解器筛选动作组合，再回到 DDS&OP / DDMRP 白盒引擎重算。 |
...
| Gurobi / OR-Tools 入口在网络结构评分工作台中展示 | 容易让用户误以为网络结构评分可以直接决定方案或生成计划 | 已处理：网络结构评分工作台隐藏候选动作组合选择入口；求解器只作为后续 DDS&OP 多方案比较阶段的候选筛选器 |
...
- Gurobi / OR-Tools 求解器适配器作为后续能力保留在独立组合选择架构中，但当前不作为网络结构评分主流程入口。
- 候选动作组合选择的正确产品位置是 DDS&OP 多方案比较阶段。网络结构评分只输出候选点、证据链和不采纳风险；DDS&OP 负责把候选转换为可重算的场景动作并执行 DDMRP 白盒回算。
