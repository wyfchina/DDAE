...
public sealed class BufferTrendWorkspaceService
{
        ...
        BufferZones zones)
    {
        ...
        var attributes = new List<SingleSkuAttribute>
        {
            ...
            new("DDMRP 参数", "缓冲配置档案", sku.BufferProfile, "用于解释该 SKU 的红黄绿区定容策略。"),
            ...
            new("DDMRP 参数", "DAF", decimal.Round(sku.DemandAdjustmentFactor, 2).ToString(System.Globalization.CultureInfo.InvariantCulture), "需求调整因子，用于把已知业务事件折算进缓冲定容。"),
            ...
            new("缓冲参数", "ADU", decimal.Round(sku.Adu, 1).ToString(System.Globalization.CultureInfo.InvariantCulture), "平均日用量，是红黄绿区定容的基础输入。"),
            ...
            new("缓冲参数", "红 / 黄 / 绿上沿", $"{decimal.Round(zones.TopOfRed, 0)} / {decimal.Round(zones.TopOfYellow, 0)} / {decimal.Round(zones.TopOfGreen, 0)}", "按 DDMRP 定容公式计算的缓冲区边界。"),
