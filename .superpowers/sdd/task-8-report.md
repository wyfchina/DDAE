...
- Rejects empty/whitespace, `NaN`, and infinite zone/demand/threshold evidence before numeric conversion. Invalid zone weeks now show `缓冲区证据缺失`; demand and threshold retain their separate Chinese warnings.
- Renders a singleton valid zone segment as three visible red/yellow/green boundary markers with period/value titles instead of three invisible zero-area paths. Multi-period segments remain smooth stacked mountains.
...
- Formal-review invalid-evidence RED: the real renderer fixture showed missing zone evidence had no visible warning; blank chart strings were still accepted as zero while `NaN`/Infinity were rejected.
- Formal-review invalid-evidence GREEN: blank/non-finite zone, demand, and threshold values now split their own evidence segments, create no zero marker/geometry, emit no `NaN`/`Infinity`, and show the appropriate Chinese warning; the suite returned to `143/143`.
- Formal-review singleton-zone RED: one valid zone week still produced three fill-only zero-area paths and failed `one valid zone week should not masquerade as a visible stacked area`.
- Formal-review singleton-zone GREEN: a one-period segment now produces exactly three visible boundary markers with backend date/value titles and no area path; the suite returned to `143/143`.
...
7. Clones the real DTO to verify zone, demand, full/partial threshold, and singleton evidence gaps are never zero-filled or bridged, including blank, `NaN`, and infinite renderer inputs.
...
- CSS delimiter check: `579/579` braces.
...
The later formal post-commit review found two additional Important evidence-visibility gaps: blank strings were still chart-valid, and singleton zone segments were fill-only degenerate areas. Both now have real-renderer RED/GREEN coverage. A fresh independent fix review confirmed both findings closed and reported:
